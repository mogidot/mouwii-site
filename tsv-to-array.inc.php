			...
			if (!empty($line) && (!$require_at_least_two_columns || (count($line) > 1))) {
				...
				if ($header_row && ($row == 1)) {
					...
					if ($header_row && (count($line) != count($data['headers']))) {
						if ($allow_mismatched_headers) {
							...
							while (count($line) > count($data['headers'])) { $data['headers'][] = '?'; }
						}
						else { return csv_to_array_error('There is a mismatch in column quantity on row '.$row.'. The header row has '.count($data['headers']).' columns, but row '.$row.' has '.count($line).'.'); }
					}
					$data[$row] = array();
				}
				...
				for ($c=0; $c<count($line); $c++) {
					...
					if ($header_row && ($row == 1)) { // if this is part of the header row
						...
						if (in_array($value, $data['headers'])) { return csv_to_array_error('There are duplicate values in the header row: '.htmlspecialchars($value).'.'); }
